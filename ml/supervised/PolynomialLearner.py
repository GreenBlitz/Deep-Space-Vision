import numpy as np
from ml.supervised.NonLinearityFunctions import linear


def calc_theta_num(n, k) -> int:
    return int(np.math.factorial(n+k)/(np.math.factorial(k)*np.math.factorial(n)))
...
class Polynomial:
    def __init__(self, dim, deg, func):
        ...
        self.dimension = dim
        self.function = func
        self.theta = [np.random.rand() for _ in range(calc_theta_num(dim, deg))]
    ...
    def __call__(self, point):
        ...
        for i in range(len(self.theta)):
            ...
            for n in range(len(degrees_arr)):
            ...
            if degrees_arr[-1] == deg:
                ...
                for j in range(len(degrees_arr) - 2, -1, -1):
                    if degrees_arr[j] != 0:
                        degrees_arr[j] -= 1
                        degrees_arr[j + 1] += 1
                        for t in range(j+2, len(degrees_arr)):
    ...
    def theta_x(self, point, index):
        ...
        for i in range(calc_theta_num(self.dimension, deg), index):
            for j in range(len(degree_arr)-2,-1,-1):
                if degree_arr[j] != 0:
                    degree_arr[j] -= 1
                    degree_arr[j+1] += 1
                    for t in range(j+2, len(degree_arr)):
                        degree_arr[j+1] += degree_arr[t]
                        degree_arr[t] = 0
                    break
        thx = 1
        for k in range(len(degree_arr)):
...
class PolynomialLearner:
    def __init__(self, out, dim=None, deg=None, kernel=None, functions=None):
        ...
        self.polynomials = [Polynomial(dim=self.dimension, deg=self.degree, func=functions[i]) for i in range(out)]
        self.outputs = out
