...
filter_contours = PipeLine((lambda cnts: filter(lambda c: cv2.contourArea(c) >= 6.0, cnts))) + list
...
contours_to_rects = PipeLine(lambda cnts: map(lambda x: cv2.boundingRect(x), cnts)) + list

contours_to_rects_sorted = contours_to_rects + (lambda rects: sorted(rects, key=lambda x: x[2] * x[3], reverse=True)) + list

contours_to_circles = PipeLine(lambda cnts: map(lambda x: cv2.minEnclosingCircle(x), cnts)) + list

contours_to_circles_sorted = contours_to_circles + (lambda rects: sorted(rects, key=lambda x: x[1], reverse=True)) + list

contours_to_rotated_rects = PipeLine(lambda cnts: map(lambda x: cv2.minAreaRect(x), cnts)) + list
