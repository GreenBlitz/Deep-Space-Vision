...
class RedDetector:
    ...
    def __init__(self):
        ...
        self.__hsv_threshold_hue = [163.0, 180.0]
        self.__hsv_threshold_saturation = [89.0, 255.0]
        self.__hsv_threshold_value = [69.0, 167.0]
...
#
    ...
    red_detector.process(im)
    thr = red_detector.hsv_threshold_output
...
def main():
    ...
    while True:
            # Read a new frame
            ok, frame = video.read()
            a = np.array(find_red_square(frame,125))
            #x = a[:,0]
            #y = a[:,1]
            ...
            #print(str(polydp)+"\n\n")
                polydp = map(lambda x: x[0], polydp)
                polydp = list(map(tuple, polydp))
                if len(polydp) >= 4:
                    print(42/(((polydp[0][0]-polydp[1][0])**2+(polydp[0][1]-polydp[1][1])**2)**0.5)*4*6)
