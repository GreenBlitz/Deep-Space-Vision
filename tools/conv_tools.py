...
def find_red_square(im:np.ndarray, threshhold=100):
    im = im/np.linalg.norm(im, axis=2).reshape(im.shape[0:2] + (1,))*255

    im2 = red(im) - (green(im))*0.6 - blue(im)*0.60
    im2 = relu(im2).astype('uint8')
    blur = cv2.GaussianBlur(im2, (5, 5), 0)
    thr = edges(cv2.threshold(blur, threshhold, 255, cv2.THRESH_BINARY)[1])
...
def small_edges(im:np.ndarray) -> np.ndarray:
